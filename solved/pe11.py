...
def main(grid, seq_len):
    ...
    for row in grid:
        for i in range(size - seq_len + 1):
            ...
            for j in range(seq_len):
                product *= row[i+j]
            top_product = max(top_product, product)

    # Verticals
    for row in range(size - seq_len + 1):
    ...
    for row in range(size - seq_len + 1):
        for col in range(size - seq_len + 1):
            # Positive Diagonals
            product = 1
            for j in range(seq_len):
                product *= grid[row+j][col+seq_len-1 - j]
            top_product = max(top_product, product)
